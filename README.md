...
No apps ship pre-installed. You describe what you need, and the OS generates it, sandboxes it, and runs it — with capability-based security ensuring generated code can only do what you approve. The system scales its intelligence to available resources: regex-only with zero models, smart routing with a tiny LLM, full generation with local or cloud models.

> **Status:** Working prototype — floating window manager, WASM sandbox, cryptographic capability tokens, app registry, Docker process apps with hardened containers, persistent storage, LLM gateway with dynamic model selection, intelligent prompt routing (LLM + regex), resource monitor, known app templates (SSH client, Chromium browser with KasmVNC), persistent sessions, static analyzer, self-improvement scheduler, and bootable VM images (Alpine + Docker). 355 tests, zero runtime dependencies. Looking for contributors. [See screenshots.](https://dayzander.github.io/llm-os/#progress)
...
```
┌──────────────────────────────────────────────────────┐
│  Generated Apps (sandboxed iframes / WASM / Docker)   │
├──────────────────────────────────────────────────────┤
│  App Runtime & Window Manager                         │
├──────────────────────────────────────────────────────┤
│  LLM OS Kernel                                        │
│  ┌──────────┬──────────┬──────────┬────────────────┐  │
│  │ Prompt   │ Resource │ LLM      │ Sandbox        │  │
│  │ Router   │ Monitor  │ Gateway  │ Manager        │  │
│  ├──────────┼──────────┼──────────┼────────────────┤  │
│  │ Cap-     │ App      │ Static   │ Known App      │  │
│  │ Based    │ Regis-   │ Analyzer │ Templates      │  │
│  │ Security │ try      │          │ (SSH, Browser)  │  │
│  └──────────┴──────────┴──────────┴────────────────┘  │
├──────────────────────────────────────────────────────┤
...
#### 1. LLM Gateway + Prompt Router + Resource Monitor
The single chokepoint for all AI generation, with intelligent routing and dynamic model selection:

- **Prompt Router** — a small LLM (0.5B–1B) classifies every prompt into type (iframe/process), complexity, template match, and model hint. Falls back to regex if no LLM is available.
- **Resource Monitor** — probes Ollama `/api/tags` and cloud API keys every 60 seconds, ranks models by tier (1–9), and auto-selects the strongest model for each task.
- **Model hints** — users can say "make a calculator using opus" or "ssh client, local" to pick a specific model or provider.
- **Known app templates** — SSH client, Chromium browser, NanoClaw skip LLM generation entirely and launch pre-defined Docker blueprints.
...
- **Any OpenAI-compatible API** — vLLM, LM Studio, llama.cpp, and custom endpoints via `OPENAI_BASE_URL`
...
The OS adapts to available resources:

| Resources | Routing | Generation | Templates |
|-----------|---------|------------|-----------|
| No models | Regex keyword matching | Unavailable | SSH, browser, NanoClaw work |
| Tiny LLM (0.5B) | LLM classifies prompts | Unavailable | + smart type/complexity detection |
| Local LLM (7B–14B) | LLM routing | Local generation | + full app generation |
| Cloud API keys | LLM routing | Best model per task | + model hints ("using opus") |

The resource monitor probes every 60s and auto-selects. Add a GPU with Ollama → the OS gets smarter. Add a Claude API key → it gets smarter again. Remove them → it degrades gracefully.
...
User types: "I need a pomodoro timer with break reminders"
                    ...
                    ▼
   ┌─── Prompt Router ──────┐
   │ LLM classify (or regex) │
   │ type=iframe, simple     │
   │ Extract model hints     │
   │ Match known templates?  │──→ Template match → launch Docker blueprint
   └────────────────────────┘
                    │
                    ▼
   ┌─── Registry Lookup ───┐
...
| Prompt Router | LLM classification + regex fallback | Implemented |
| Resource Monitor | Dynamic model probing + tier ranking | Implemented |
| Local LLM | Ollama | Implemented |
| Cloud LLM | Claude, OpenAI, OpenRouter, Groq, Together | Implemented (pluggable) |
| Known App Templates | SSH client, Chromium browser, NanoClaw | Implemented |
...
```bash
npm test                   # 355 tests across 8 test files
```
...
### Phase 3a: Desktop Experience + Security Hardening (in progress)
...
- [x] Floating window manager (drag, resize, minimize, maximize, z-index stacking)
- [x] 12 built-in example apps (calculator, todo, notepad, pomodoro, world clock, etc.)
- [x] App deletion and user ratings
- [x] Per-app markdown spec for AI-driven improvements
- [x] Shell self-improvement system (versioned UI, LLM-driven improvements with rollback)
- [x] Docker container hardening (read-only rootfs, no-new-privileges, memory/PID limits)
- [x] SSH client process app with persistent sessions (survive page refresh)
- [x] Chromium browser process app (KasmVNC streaming)
- [x] Intelligent prompt router (LLM classification with regex fallback)
- [x] Resource monitor (dynamic model probing, tier-based selection)
- [x] Model hint extraction ("using opus", "with haiku", "via local")
- [x] Known app templates (SSH, browser, NanoClaw — skip LLM generation)
- [x] App session persistence across page refreshes
...
- [ ] Resource quotas for Docker containers (CPU, memory, network limits per app)
